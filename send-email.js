...
function firebase() {
  if (!admin.apps.length) {
    ...
    if (raw && !raw.trim().startsWith("{")) raw = Buffer.from(raw, "base64").toString("utf8"); // also accepts base64
...
module.exports = async (req, res) => {
  ...
  let fb;
  try { fb = firebase(); } catch (e) { return res.status(500).json({ ok: false, error: "bad-service-account" }); }
